...
impl LibraryState {
    ...
    pub fn selected_track(&self) -> Option<&Track> {
        self.list_state.selected().and_then(|i| self.display_tracks().get(i))
    }
    ...
    pub fn jump_top(&mut self) {
        if !self.display_tracks().is_empty() { self.list_state.select(Some(0)); }
    }
    ...
    pub fn render(&mut self, frame: &mut Frame, area: Rect, focused: bool) {
        ...
        let items: Vec<ListItem> = display.iter().map(|t| {
